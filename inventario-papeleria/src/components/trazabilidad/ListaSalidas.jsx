...
export default function ListaSalidas({ rol = "gestor", showGestor = true }) {
  ...
  const getDetalleUrl = (idSalida) =>
    rol === "admin" ? `/admin/salida/${idSalida}` : `/gestor/salida/${idSalida}`;
          ...
          <tbody>
            {loading ? (
              ...
              filteredSalidas.map((salida, index) => (
                <tr
                  key={salida.idSalida}
                  onClick={() => navigate(getDetalleUrl(salida.idSalida))}
        ...
        {loading ? (
          ...
          filteredSalidas.map((salida) => (
            ...
            >
              ...
              <p className="text-sm text-gray-600">
                <strong>Gestor:</strong> {salida.usuarioGestor?.nombre} {salida.usuarioGestor?.apellidos}
              </p>
            </div>
          ))
        ) : (
